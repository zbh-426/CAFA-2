...
def solve(
    ...
    # CAFA-SEMANTIC-BINDING-PATCH START
    # Run deterministic semantic middle-end repairs before CAFA-AST lowering.
    # This keeps the main architecture unchanged: the compiler still lowers the
    # normalized IR expressions into CAFA-AST and then to Gurobi, but common
    # semantic binding mistakes are patched without calling the LLM verifier.
    try:
        ir, semantic_repair_reasons = deterministic_semantic_repair(description, ir)
        ...
        # IR so the previous pipeline behavior is preserved.
