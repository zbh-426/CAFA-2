...
Pipeline:
  description
    -> LLM evidence-bound surface JSON              # model-facing parse artifact, not true IR
    -> parse/normalize CAFA-Surface formulation
    -> deterministic front-end semantic repair
    -> build typed CAFA-AST                         # canonical compiler representation
    -> semantic accounting checks over AST
    -> lower CAFA-AST to CAFA-Linear-IR             # compiler middle/lower representation
    -> deterministic Linear-IR-to-Gurobi compiler
    -> execute Gurobi -> metrics

Design choice:
  To recover the strong behavior of the original CAFA prompt, the LLM still emits the familiar
  CAFA JSON fields: variables, objective, constraints, and a code string.  This version treats that
  JSON as a *surface formulation*, not as the compiler IR.  The generated code remains only a
  scaffold/debug hint and is NOT executed.  The executable Gurobi program is generated
  deterministically after AST construction and AST-to-IR lowering.
...
SYSTEM_PROMPT = """You are an expert OR modeler. Convert the problem into an evidence-bound compact CAFA-Surface JSON.
...
def parse_formulation(raw: str) -> Optional[dict]:
    """Backward-compatible name: parse and normalize CAFA-Surface, or return None."""
    try:
...
def parse_ir_lite(raw: str) -> dict:
    """Backward-compatible wrapper for parse_surface_formulation().

    CAFA-COMPILER-LIKE-CHANGE:
    Older versions called the model-facing JSON "IR-lite".  In a stricter
    compiler workflow this object is only a surface formulation / parse artifact.
    The real compiler IR is produced later by lower_ast_to_linear_ir().
    """
    return parse_surface_formulation(raw)


def parse_surface_formulation(raw: str) -> dict:
    """Parse model output into a normalized evidence-bound CAFA-Surface object.

    CAFA-COMPILER-LIKE-CHANGE:
    The LLM output is now explicitly treated as a surface DSL.  It preserves the
    small-model-friendly CAFA fields but is not the canonical compiler IR.
    ...
    ir = {
        "surface_version": "cafa-surface-evidence-v1",
...
# All deterministic code generation should go through AST -> Linear-IR -> Gurobi.
...
# solve() pipeline below now compiles from the lowered Linear-IR.


def _ast_terms_to_coeffs(expr_ast: dict) -> dict[str, float]:
    """Convert a LinearExpr AST node into a coefficient dictionary."""
    coeffs: dict[str, float] = {}
    for t in expr_ast.get("terms", []) if isinstance(expr_ast, dict) else []:
        var = str(t.get("var", ""))
        try:
            coef = float(t.get("coef", 0.0))
        except Exception:
            continue
        if var and math.isfinite(coef) and abs(coef) >= 1e-12:
            coeffs[var] = coeffs.get(var, 0.0) + coef
    return {k: v for k, v in coeffs.items() if abs(v) >= 1e-12}


def lower_ast_to_linear_ir(ast_model: dict) -> dict:
    """Lower canonical CAFA-AST into a compact linear optimization IR.

    The IR is intentionally solver-independent: it contains typed variables, a
    linear objective, and normalized linear constraints.  This is closer to a real
    compiler IR than the previous model-facing IR-lite JSON.
    """
    if not isinstance(ast_model, dict) or ast_model.get("kind") != "Model":
        raise IRValidationError("lower_ast_to_linear_ir expects a CAFA-AST Model")
    ...
    for v in ast_model.get("variables", []):
        ...
        variables.append({
            ...
            "vtype": str(v.get("vtype") or "CONTINUOUS").upper(),
            "lb": float(v.get("lb", 0.0) or 0.0),
            "ub": v.get("ub"),
            "evidence_ids": list(v.get("evidence_ids", [])),
        })
    if not variables:
        raise IRValidationError("linear IR has no variables")

    obj_expr = ast_model.get("objective", {}).get("expr", {})
    objective = {
        "sense": ast_model.get("sense"),
        "coeffs": _ast_terms_to_coeffs(obj_expr),
        "constant": float(obj_expr.get("constant", 0.0) or 0.0),
        "evidence_terms": ast_model.get("objective", {}).get("terms_from_model", []),
    }

    constraints: list[dict] = []
    for i, c in enumerate(ast_model.get("constraints", [])):
        if not isinstance(c, dict):
            continue
        ...
        constraints.append({
            "name": c.get("name") or f"c{i+1}",
            "coeffs": _ast_terms_to_coeffs(lhs),
            "sense": norm.get("sense"),
            "rhs": float(norm.get("rhs", 0.0) or 0.0),
            "source": c.get("source", ""),
            "evidence_ids": list(c.get("evidence_ids", [])),
        })
    if not constraints:
        raise IRValidationError("linear IR has no constraints")

    return {
        "ir_version": "cafa-linear-ir-v1",
        "source_ast_version": ast_model.get("ast_version"),
        "problem_type": ast_model.get("problem_type"),
        "sense": ast_model.get("sense"),
        "evidence": ast_model.get("evidence", []),
        "variables": variables,
        "objective": objective,
...
def compile_linear_ir_to_gurobi(linear_ir: dict, save_path: Optional[str] = None) -> str:
    """Compile solver-independent CAFA-Linear-IR into full runnable Gurobi code."""
    if not isinstance(linear_ir, dict) or linear_ir.get("ir_version") != "cafa-linear-ir-v1":
        raise IRValidationError("compile_linear_ir_to_gurobi expects cafa-linear-ir-v1")

    var_ids = [str(v["id"]) for v in linear_ir.get("variables", [])]
    ...
    for v in linear_ir["variables"]:
        pyv = var_map[v["id"]]
        name = py_quote(v.get("name") or v["id"])
        vtype = str(v.get("vtype", "CONTINUOUS")).upper()
        lb = float(v.get("lb", 0.0) or 0.0)
        ub = v.get("ub")
        ub_arg = "" if ub is None else f", ub={float(ub):.15g}"
        if vtype == "CONTINUOUS":
            lines.append(f"{pyv} = m.addVar(name={name}, lb={lb:.15g}{ub_arg}, vtype=gp.GRB.CONTINUOUS)")
        elif vtype == "INTEGER":
            lines.append(f"{pyv} = m.addVar(name={name}, lb={lb:.15g}{ub_arg}, vtype=gp.GRB.INTEGER)")
        elif vtype == "BINARY":
    ...
    lines.append("")
    obj = linear_ir.get("objective", {})
    obj_expr = linear_to_gurobi(obj.get("coeffs", {}), float(obj.get("constant", 0.0) or 0.0), var_map)
    grb_sense = "gp.GRB.MAXIMIZE" if linear_ir.get("sense") == "MAXIMIZE" else "gp.GRB.MINIMIZE"
    lines.append(f"m.setObjective({obj_expr}, {grb_sense})")

    for i, c in enumerate(linear_ir.get("constraints", [])):
        sense = str(c.get("sense", ""))
        if sense not in {"<=", ">=", "=="}:
            raise IRValidationError(f"bad constraint sense in linear IR: {sense}")
        lhs_expr = linear_to_gurobi(c.get("coeffs", {}), 0.0, var_map)
        cname = py_quote(c.get("name") or f"c{i+1}")
        rhs = float(c.get("rhs", 0.0) or 0.0)
...
def compile_ast_to_gurobi(ast_model: dict, save_path: Optional[str] = None) -> str:
...
# Stronger but still conservative discrete and continuous unit vocabularies.
# These extend the existing DISCRETE_NOUNS / CONTINUOUS_NOUNS lists without
# changing the original lists in-place.
SEMANTIC_DISCRETE_UNITS = {
    "pill", "pills", "tablet", "tablets", "capsule", "capsules",
    "phone", "phones", "laptop", "laptops", "appliance", "appliances",
    "car", "cars", "model", "models", "furnace", "furnaces",
    "container", "containers", "unit", "units", "item", "items",
    "product", "products", "package", "packages", "box", "boxes",
    "truck", "trucks", "chair", "chairs", "desk", "desks",
    "worker", "workers", "employee", "employees", "nurse", "nurses",
}

SEMANTIC_CONTINUOUS_UNITS = {
    "hour", "hours", "time", "acre", "acres", "ml", "liter", "liters",
    "litre", "litres", "gram", "grams", "kg", "kilogram", "kilograms",
    "pound", "pounds", "sq", "ft", "sqft", "space", "area", "volume",
    "water", "electricity", "kwh", "money", "budget", "dollar", "dollars",
}

COST_CUES = ["cost", "costs", "spend", "budget", "price", "priced", "expense", "expenses"]
PROFIT_CUES = ["profit", "profits", "earn", "earns", "revenue", "return"]
SPACE_CUES = ["space", "floor", "sq ft", "sqft", "square feet"]
GROUP_TOTAL_CUES = [
    "in total", "total", "combined", "altogether", "of either type",
    "of all", "all appliances", "all products", "all cars", "maximum of",
    "can make a maximum", "at most", "no more than",
]
...
def _entity_aliases_from_text(text: str) -> list[str]:
    toks = [_singular_token(t) for t in re.findall(r"[A-Za-z0-9]+", text.lower())]
    toks = [t for t in toks if t and t not in {"the", "a", "an", "of", "per", "each", "and"}]
    aliases: list[str] = []
    for t in toks:
        if t not in aliases:
            aliases.append(t)
    joined = " ".join(toks)
    if joined and joined not in aliases:
        aliases.append(joined)
    return aliases


def _evidence_text_for_var(ir: dict, var: dict) -> str:
    evidence_by_id = _evidence_map(ir)
    ids = list(var.get("evidence_ids", [])) if isinstance(var, dict) else []
    return _evidence_text_for_ids(
        evidence_by_id,
        ids,
        extra=f"{var.get('name', '')} {var.get('rationale', '')}",
    )


def _infer_entity_phrase_from_description(description: str, var_id: str, var_name: str) -> str:
    """Infer a textual entity phrase for generic variables such as x1/x2.

    This is intentionally heuristic and only used for binding checks/repairs.
    Existing model symbols are still kept unchanged.
    """
    desc = str(description or "")
    vid = str(var_id or "")
    # Pattern: "x1 regular models per day", "x2 premium models per day".
    if vid:
        pat = re.compile(rf"\b{re.escape(vid)}\s+([A-Za-z][A-Za-z\- ]{{0,40}}?)(?:\s+per\b|[,.;()]|\band\b|$)", re.I)
        m = pat.search(desc)
        if m:
            phrase = m.group(1).strip()
            # Keep the most informative 1-3 tokens, dropping generic tails.
            phrase = re.sub(r"\b(per|day|unknown|values?|both|greater|than|or|equal|to)\b.*$", "", phrase, flags=re.I).strip()
            if phrase:
                return phrase
    # Non-generic names are already useful entity labels.
    if var_name and _safe_lower(var_name) not in GENERIC_VAR_NAMES:
        return str(var_name)
    return str(var_name or var_id)


def build_semantic_symbol_table(description: str, ir: dict) -> dict[str, dict]:
    """Build a lightweight compiler symbol table: var -> entity aliases/unit hints."""
    table: dict[str, dict] = {}
    for v in ir.get("variables", []):
        ...
        ev_text = _evidence_text_for_var(ir, v)
        phrase = _infer_entity_phrase_from_description(description, vid, vname)
        aliases = []
        for chunk in [vid, vname, phrase, ev_text]:
            for a in _entity_aliases_from_text(chunk):
...
def _classify_relation_role(context: str) -> str:
    t = context.lower()
    if _contains_any(t, COST_CUES):
        return "cost"
    if _contains_any(t, PROFIT_CUES):
        return "profit"
    if _contains_any(t, SPACE_CUES):
        return "space"
    if "water" in t:
        return "water"
    if "electricity" in t or "kwh" in t:
        return "electricity"
    if "contains" in t or "requires" in t or "consumes" in t or "make" in t:
        return "coefficient"
    return "other"


def _number_positions(text: str) -> list[tuple[int, int, float]]:
    out: list[tuple[int, int, float]] = []
    for m in NUMBER_RE.finditer(text):
        vals = extract_numbers(m.group(0))
        if vals:
            out.append((m.start(), m.end(), vals[0]))
    return out


def _alias_regex(alias: str) -> Optional[re.Pattern]:
    alias = alias.strip().lower()
    if not alias or len(alias) <= 1:
        return None
    # Allow simple plural form for single-token aliases.
    if " " not in alias and alias.isalpha():
        return re.compile(rf"\b{re.escape(alias)}s?\b", re.I)
    return re.compile(rf"\b{re.escape(alias)}\b", re.I)


def extract_semantic_relations(description: str, symbol_table: dict[str, dict]) -> list[dict]:
    """Extract approximate typed numeric relations from the source text.

    This is not a full parser.  It is a compiler middle-end guard used to catch
    high-value errors where the model used the right numbers but attached them
    ...
    for sent_start, sent_end, sent in spans:
        sent_nums = _number_positions(sent)
        if not sent_nums:
            continue
        sent_role = _classify_relation_role(sent)
        mentions: list[tuple[int, int, str, str]] = []  # local start/end, var, alias
        for var, info in symbol_table.items():
            # Prefer entity/name aliases and avoid matching raw generic ids such as x or y.
            aliases = [a for a in info.get("aliases", []) if a not in GENERIC_VAR_NAMES and len(a) > 1]
            for alias in aliases:
                rgx = _alias_regex(alias)
                if rgx is None:
                    continue
                for m in rgx.finditer(sent):
                    mentions.append((m.start(), m.end(), var, alias))
        if not mentions:
            continue
        mentions.sort(key=lambda x: x[0])

        for mi, (ms, me, var, alias) in enumerate(mentions):
            next_m_start = mentions[mi + 1][0] if mi + 1 < len(mentions) else len(sent)
            prev_m_end = mentions[mi - 1][1] if mi > 0 else 0
            # Prefer a number after the mention and before the next entity mention.
            candidate_nums = [(ns, ne, val) for ns, ne, val in sent_nums if me <= ns <= next_m_start]
            # Fallback: number immediately before mention in the same local clause.
            if not candidate_nums:
                candidate_nums = [(ns, ne, val) for ns, ne, val in sent_nums if prev_m_end <= ns <= ms]
            if not candidate_nums:
                continue
            # Choose the closest number to the mention boundary.
            ns, ne, val = min(candidate_nums, key=lambda z: min(abs(z[0] - me), abs(ms - z[1])))
            window_start = max(0, min(ms, ns) - 80)
            window_end = min(len(sent), max(me, ne) + 80)
            context = sent[window_start:window_end]
            role = _classify_relation_role(context)
            if role == "other":
                role = sent_role
            relations.append({
                "subject": var,
    ...
    # Deduplicate same subject/role/value.
    deduped: list[dict] = []
    seen: set[tuple[str, str, float]] = set()
    for r in relations:
        key = (str(r.get("subject")), str(r.get("role")), round(float(r.get("value", 0.0)), 10))
        if key not in seen:
            seen.add(key)
            deduped.append(r)
    return deduped
...
def _constraint_evidence_text(ir: dict, c: dict) -> str:
    evidence_by_id = _evidence_map(ir)
    return _evidence_text_for_ids(evidence_by_id, list(c.get("evidence_ids", [])), extra=str(c.get("source", "")))


def _infer_constraint_role(description: str, ir: dict, c: dict) -> str:
    text = f"{_constraint_evidence_text(ir, c)} {c.get('expression', '')}".lower()
    if _contains_any(text, COST_CUES) or "budget" in text:
        return "cost"
    if _contains_any(text, SPACE_CUES):
        return "space"
    if "water" in text:
        return "water"
    if "electricity" in text or "kwh" in text:
        return "electricity"
    if _contains_any(text, PROFIT_CUES):
        return "profit"
    return "other"


def _format_coef_var(coef: float, var: str) -> str:
    coef = float(coef)
    if abs(coef - 1.0) < 1e-12:
        return str(var)
    if abs(coef + 1.0) < 1e-12:
        return f"-{var}"
    return f"{coef:.15g}*{var}"


def _format_linear_expr_simple(coeffs: dict[str, float], const: float = 0.0) -> str:
    parts = [_format_coef_var(coef, var) for var, coef in coeffs.items() if abs(float(coef)) > 1e-12]
    if abs(float(const)) > 1e-12 or not parts:
        parts.append(f"{float(const):.15g}")
    expr = " + ".join(parts)
    expr = expr.replace("+ -", "- ")
    return expr


def _relation_value_for_var(relations: list[dict], var: str, role: str) -> Optional[float]:
    vals = [float(r["value"]) for r in relations if r.get("subject") == var and r.get("role") == role]
    if not vals:
        return None
    # If several candidates exist, use the last textual mention.  This works well
    # for sentences such as "phone costs $400..., laptop, $100" after dedupe.
    return vals[-1]


def _patch_variable_types(description: str, ir: dict, symbol_table: dict[str, dict]) -> list[str]:
    notes: list[str] = []
    desc_l = _safe_lower(description)
    global_continuous_context = "how many hours" in desc_l or "hours should" in desc_l
    for v in ir.get("variables", []):
        if not isinstance(v, dict):
            continue
        vid = str(v.get("id") or "")
        ...
        target = " ".join([
            str(v.get("name", "")), str(v.get("rationale", "")),
            str(info.get("entity_phrase", "")), str(info.get("evidence_text", "")),
        ]).lower()
        has_discrete = _contains_any(target, sorted(SEMANTIC_DISCRETE_UNITS))
        has_continuous = _contains_any(target, sorted(SEMANTIC_CONTINUOUS_UNITS)) or global_continuous_context
        # Also infer from source if the entity appears near a strong discrete unit.
        for alias in info.get("aliases", []):
            if alias in GENERIC_VAR_NAMES or len(alias) <= 1:
                continue
            if re.search(rf"\b(pill|tablet|capsule|phone|laptop|car|furnace|container)s?\s+(?:of\s+)?{re.escape(alias)}\b", desc_l):
                has_discrete = True
            if re.search(rf"\b{re.escape(alias)}s?\b[^.]*\b(pill|tablet|capsule|phone|laptop|car|furnace|container)s?\b", desc_l):
                has_discrete = True
        old = str(v.get("vtype", "CONTINUOUS")).upper()
        if old == "CONTINUOUS" and has_discrete and not has_continuous:
            v["vtype"] = "INTEGER"
            notes.append(f"deterministic_repair:vtype:{vid}:CONTINUOUS->INTEGER:discrete_unit_evidence")
    return notes


def _patch_cost_coefficient_bindings(description: str, ir: dict, symbol_table: dict[str, dict], relations: list[dict]) -> list[str]:
    notes: list[str] = []
    var_ids = {str(v.get("id")) for v in ir.get("variables", []) if isinstance(v, dict) and v.get("id")}
    if not var_ids:
        return notes
    ...
    for c in ir.get("constraints", []):
        if not isinstance(c, dict):
            continue
        role = _infer_constraint_role(description, ir, c)
        # Keep this deterministic patch conservative: only cost/budget swaps.
        if role != "cost":
            continue
        try:
            lhs_s, sense, rhs_s = split_constraint(str(c.get("expression", "")))
            lhs_coeffs, lhs_const = parse_linear_expr(lhs_s, var_ids)
            rhs_coeffs, rhs_const = parse_linear_expr(rhs_s, var_ids)
        except Exception:
            continue
        if rhs_coeffs or abs(lhs_const) > 1e-12:
            continue
        if len(lhs_coeffs) < 2:
            continue
        expected: dict[str, float] = {}
        for var in lhs_coeffs:
            val = _relation_value_for_var(relations, var, "cost")
            if val is not None:
                expected[var] = val
        if set(expected) != set(lhs_coeffs):
            continue
        old_vals = sorted(abs(float(v)) for v in lhs_coeffs.values())
        new_vals = sorted(abs(float(v)) for v in expected.values())
        same_number_pool = len(old_vals) == len(new_vals) and all(_num_close(a, b, rel=1e-7, abs_tol=1e-7) for a, b in zip(old_vals, new_vals))
        mismatches = [var for var in lhs_coeffs if not _num_close(abs(float(lhs_coeffs[var])), abs(float(expected[var])), rel=1e-7, abs_tol=1e-7)]
        if not mismatches or not same_number_pool:
            continue
        # Preserve coefficient signs from original expression but replace magnitudes.
        patched_coeffs = {
            var: math.copysign(abs(float(expected[var])), float(lhs_coeffs[var]) if abs(float(lhs_coeffs[var])) > 1e-12 else 1.0)
            for var in lhs_coeffs
        }
        old_expr = str(c.get("expression", ""))
        c["expression"] = f"{_format_linear_expr_simple(patched_coeffs)} {sense} {rhs_const:.15g}"
        c["source"] = (str(c.get("source", "")) + " | deterministic coefficient-variable binding repair").strip(" |")
        notes.append(f"deterministic_repair:coefficient_binding:{c.get('name')}: {old_expr} -> {c['expression']}")
    return notes


def _is_single_var_upper_bound(expr: str, var_ids: set[str]) -> Optional[tuple[str, float]]:
    try:
        coeffs, sense, rhs = parse_constraint_expr(expr, var_ids)
    except Exception:
        return None
    if sense != "<=" or len(coeffs) != 1:
        return None
    var, coef = next(iter(coeffs.items()))
    if not _num_close(float(coef), 1.0, rel=1e-7, abs_tol=1e-7):
        return None
    return var, float(rhs)


def _patch_group_scope_bounds(description: str, ir: dict) -> list[str]:
    notes: list[str] = []
    ...
    for i, c in enumerate(constraints):
        if not isinstance(c, dict):
            continue
        parsed = _is_single_var_upper_bound(str(c.get("expression", "")), var_ids)
        if parsed is None:
            continue
        var, rhs = parsed
        # Do not touch small per-item binary-style bounds.
        if rhs <= 1.0:
            continue
        key = round(rhs, 10)
        groups.setdefault(key, []).append((i, var))

    desc_l = _safe_lower(description)
    for rhs_key, items in groups.items():
        if len(items) < 2:
            continue
        source_text = " ".join(_constraint_evidence_text(ir, constraints[i]) for i, _ in items).lower()
        combined_text = f"{source_text} {desc_l}"
        has_group_cue = _contains_any(combined_text, GROUP_TOTAL_CUES) and (
            "either type" in combined_text or "in total" in combined_text or "total" in combined_text
            or "combined" in combined_text or "altogether" in combined_text or "maximum of" in combined_text
        )
        # Avoid converting explicit each/per-variable statements.
        has_each_cue = re.search(rf"\b(each|respectively)\b[^.]*\b{rhs_key:g}\b", combined_text) is not None
        if not has_group_cue or has_each_cue:
            continue
        # If the source/evidence for every bound is empty/generic, require a very
        # explicit group phrase in the original description.
        if not source_text.strip() and "either type" not in desc_l and "in total" not in desc_l:
            continue
        idxs = [i for i, _ in items]
        vars_in_group = [var for _, var in items]
        union_eids: list[str] = []
        for i in idxs:
            for eid in constraints[i].get("evidence_ids", []):
                if eid not in union_eids:
                    union_eids.append(eid)
        new_expr = f"{' + '.join(vars_in_group)} <= {rhs_key:.15g}"
        new_c = {
            "name": safe_constr_name(f"scope_total_{rhs_key:g}", len(constraints)),
            "expression": new_expr,
            "source": "deterministic scope repair: group-total capacity",
            "evidence_ids": union_eids,
        }
        old_exprs = [constraints[i].get("expression", "") for i in idxs]
        # Remove from high to low index to preserve earlier constraints.
        for i in sorted(idxs, reverse=True):
            constraints.pop(i)
        constraints.append(new_c)
        notes.append(f"deterministic_repair:constraint_scope:{old_exprs} -> {new_expr}")
        break  # one conservative scope repair per problem
    return notes


def deterministic_semantic_repair(description: str, ir: dict) -> tuple[dict, list[str]]:
    """Apply conservative deterministic semantic repairs before AST lowering."""
    patched = copy.deepcopy(ir)
    notes: list[str] = []
    symbol_table = build_semantic_symbol_table(description, patched)
    relations = extract_semantic_relations(description, symbol_table)
    notes.extend(_patch_variable_types(description, patched, symbol_table))
    # Rebuild symbol table after vtype edits so saved context is current.
    symbol_table = build_semantic_symbol_table(description, patched)
    relations = extract_semantic_relations(description, symbol_table)
    notes.extend(_patch_cost_coefficient_bindings(description, patched, symbol_table, relations))
    notes.extend(_patch_group_scope_bounds(description, patched))
    patched["semantic_symbol_table"] = symbol_table
    patched["semantic_relation_table"] = relations
...
def compile_ir_to_gurobi(ir: dict, save_path: Optional[str] = None) -> str:
    """Backward-compatible compiler wrapper.

    CAFA-COMPILER-LIKE-CHANGE:
    Older callers may still pass the old surface/IR-lite object here.  For a
    stricter compiler path, this wrapper immediately builds AST, lowers AST to
    CAFA-Linear-IR, and compiles that IR.  New code should prefer:
        ast_model = build_cafa_ast(surface)
        linear_ir = lower_ast_to_linear_ir(ast_model)
        code = compile_linear_ir_to_gurobi(linear_ir)
    """
...
def make_record(description: str, ground_truth: float) -> dict:
...
def result_is_better(candidate: dict, current: dict, ground_truth: float) -> bool:
    """Prefer a revised solution only when it successfully improves the objective error."""
